<template>
  <section class="relative w-full bg-darkblue px-6 py-24 md:px-10 md:py-32">
    ...
    <div class="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
      <h2
        class="ea-display font-serif text-beige text-3xl md:text-5xl font-normal leading-[1.1] max-w-xl"
        v-html="slice.primary.heading"
      />
      <p
        v-if="slice.primary.body"
        class="text-grey text-base leading-relaxed max-w-md"
      >
        {{ slice.primary.body }}
      </p>
    </div>
    ...
    <div class="mt-16 flex gap-4 md:gap-8">
      <!-- Y axis ticks -->
      <div class="flex flex-col-reverse justify-between text-xs text-grey py-1">
      ...
      <div class="flex-1">
        ...
        >
          ...
          <g stroke="currentColor" class="text-beige/10" stroke-dasharray="2 6">
          ...
          <path :d="gapArea" class="fill-beige/10" />
          <!-- demand curve -->
          <path :d="demandLine" fill="none" stroke="currentColor"
            class="text-beige" stroke-width="3" />
          <!-- supply curve -->
          <path :d="supplyLine" fill="none" stroke="currentColor"
            class="text-grey" stroke-width="3" stroke-dasharray="8 6" />
        </svg>

        <!-- X axis labels -->
        <div class="mt-4 flex justify-between text-xs text-grey">
      ...
      <div class="flex flex-col justify-between w-28 md:w-44 py-1">
        <div>
          <div class="font-serif text-beige text-3xl md:text-5xl leading-none">{{ demand.value }}</div>
          <div class="mt-2 text-[11px] tracking-widest text-grey">{{ demand.label }}</div>
        </div>
        <div>
          <div class="font-serif text-beige text-3xl md:text-5xl leading-none">{{ supply.value }}</div>
          <div class="mt-2 text-[11px] tracking-widest text-grey">{{ supply.label }}</div>
        </div>
      </div>
    </div>
  </section>
